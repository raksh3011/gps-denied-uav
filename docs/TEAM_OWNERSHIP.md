...
Real Mission Manager (`real_mission`, backed by `MissionManager` — a waypoint-sequencing state machine using `Waypoint.acceptance_radius` to advance through a multi-leg route, since `real_planner` only ever reads `waypoints.front()`) is implemented and unit/contract-tested, no Gazebo/PX4 needed. Default params reproduce the old golden scenario exactly (drop-in swap); a genuine multi-leg example lives at `src/mission/uav_mission/config/multi_leg_example.yaml`. One known rough edge, not silently patched: `real_planner_node` treats an empty-waypoints `Mission` (i.e. mission complete) as `STATE_FAILED` rather than a clean "done" state — see [docs/MISSION.md](MISSION.md).
