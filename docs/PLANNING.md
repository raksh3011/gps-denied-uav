...
**Fixed:** `AStarPlanner`/`ThetaStarPlanner`'s open-list had no stale-entry check — when a cell's cost was revised, the old queue entry for it was never invalidated, so popping it later re-ran a full 6-neighbor expansion (each a `traceLine` raycast in Theta*'s case) for no benefit. On a small test grid this never mattered, but on a large open grid with sparse obstacles (surfaced by the visual demo's 30x15x10m pillar arena — see `demo_mission.launch.xml`), the queue filled with stale duplicates and `max_expansions` got burned re-processing already-settled cells instead of reaching a goal that was trivially reachable — reported as "no global path found to goal" despite wide-open corridors. Fixed with the same lazy-deletion pattern `DStarLitePlanner` already used (store `g` at push time, skip a popped entry if it no longer matches the current best `g_score`). Regression test: `ThetaStarPlanner, FindsPathOnLargeOpenGridWithSparseObstacles`.
