...
## Algorithmic contribution: Margasoochi (confidence-adaptive risk margin)
...
8. **Found via `analysis/compare_planners`, not yet fixed:** `diffAndUpdateSnapshot()` does an unconditional full grid scan (O(total cells) — 288,000 at the arena's 120x60x40 scale) on every single `update()` call, regardless of how few cells actually changed. This partially undercuts the "O(changed cells), not O(grid)" incremental claim documented above — the scan itself costs real, measurable milliseconds every tick before any of the genuinely-incremental `updateVertex()` work even begins. A real fix (having the caller pass a list of changed cells, or maintaining a spatial index of "cells that could plausibly have changed" instead of a linear scan) would need real design thought, not a quick patch — tracked here rather than fixed silently.
